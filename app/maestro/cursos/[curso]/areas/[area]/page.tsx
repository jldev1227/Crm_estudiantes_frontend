...
import { OBTENER_ACTIVIDADES } from '@/app/graphql/queries/obtenerActividades';
...
export default function CursoPage() {
  ...
  const { loading: loadingCurso, error: errorCurso, data: cursoData } = useQuery(OBTENER_CURSO, {
    variables: { id, area_id }
  });

  const { loading: loadingActividades, error: errorActividades, data: actividadesData } = useQuery(OBTENER_ACTIVIDADES_POR_AREA, {
    variables: { grado_id: id, area_id }
  });

  const [eliminarActividad, { loading: loadingDelete }] = useMutation(ELIMINAR_ACTIVIDAD, {
  ...
  if (errorCurso) return <div>Error al cargar el curso: {errorCurso.message}</div>;
  if (errorActividades) return <div>Error al cargar las actividades: {errorActividades.message}</div>;

  // Verificar que los datos existan
  if (!cursoData?.obtenerCurso) return <div>No se encontró información del curso</div>;

  const curso: CursoData = cursoData.obtenerCurso;

  // Asegurarse de que actividades sea siempre un array, incluso si es undefined
  const actividades: ActividadData[] = actividadesData?.obtenerActividades || [];
        ...
        <div className="space-y-10">
          <div className="space-y-4">
            ...
            {curso.estudiantes && curso.estudiantes.length > 0 ? (
              ...
              <p className="text-yellow-500">No hay estudiantes en este curso</p>
            )}
          </div>
                  ...
                  <div key={actividad.id} className="border rounded-lg">
                    <div className="space-y-2 p-4">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">{formatearFecha(actividad.fecha)}</p>
                      </div>
