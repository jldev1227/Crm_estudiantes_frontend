...
const CalculatorIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-4">
    <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 15.75V18m-7.5-6.75h.008v.008H8.25v-.008Zm0 2.25h.008v.008H8.25V13.5Zm0 2.25h.008v.008H8.25v-.008Zm0 2.25h.008V18H8.25v-.008ZM12 13.5h.008v.008H12V13.5Zm0 2.25h.008v.008H12v-.008Zm0 2.25h.008V18H12v-.008ZM15.75 13.5h.008v.008h-.008V13.5Zm0 2.25h.008v.008h-.008v-.008ZM8.25 9.75h8.25V12h-8.25V9.75Z" />
  </svg>
);
...
const SistemaCalificaciones = () => {
  ...
  const validateCalificacionesData = (
    calificaciones: CalificacionContext[],
    ...
    const calificacionesMap = new Map(calificaciones.map(cal => [cal.estudiante_id, cal]));
    ...
    estudiantes.forEach(estudiante => {
      ...
      if (!calificacion) {
        validation.warnings.push(`Estudiante ${estudiante.nombre_completo} no tiene calificaciones`);
        return;
      }
      ...
      if (calificacion.estudiante_id !== estudiante.id) {
        validation.errors.push(`ID inconsistente para ${estudiante.nombre_completo}`);
        validation.isValid = false;
      }

      // Validar notas del estudiante
      const notasMap = new Map(calificacion.notas.map(nota => [nota.actividad_id, nota]));
      ...
      actividades.forEach(actividad => {
        ...
        if (!nota) {
          validation.warnings.push(`${estudiante.nombre_completo}: falta nota para ${actividad.nombre}`);
        ...
        if (nota.valor < 0 || nota.valor > 5) {
          validation.errors.push(`${estudiante.nombre_completo}: nota inválida (${nota.valor}) en ${actividad.nombre}`);
        ...
        if (nota.porcentaje !== actividad.porcentaje) {
          validation.warnings.push(`${estudiante.nombre_completo}: porcentaje inconsistente en ${actividad.nombre}`);
        }
      });
      ...
      const notaCalculada = calcularNotaFinal(calificacion.notas);
      const diferencia = Math.abs(notaCalculada - calificacion.notaFinal);

      if (diferencia > 0.1) { // Tolerancia de 0.1 por redondeo
    ...
    calificaciones.forEach(calificacion => {
      if (!estudiantesMap.has(calificacion.estudiante_id)) {
        validation.errors.push(`Calificación huérfana para estudiante ID: ${calificacion.estudiante_id}`);
        validation.isValid = false;
      }
    });
    ...
    if (Math.abs(totalPorcentaje - 100) > 0.1) {
      validation.errors.push(`Los porcentajes de actividades no suman 100% (suma: ${totalPorcentaje}%)`);
      validation.isValid = false;
    }
  ...
  const validation = validateCalificacionesData(calificacionesContext, estudiantes, actividades)
  ...
  const allColumns = [
    <TableColumn key="estudiante" className="bg-gray-50 sticky left-0 z-10 min-w-[150px]">
      Estudiante
    </TableColumn>,
    ...
    <TableColumn key="reporte">Reporte</TableColumn>
  ];
        ...
        <Card className="border border-yellow-200 bg-yellow-50">
          <CardBody className="py-3">
            <div className="flex items-start gap-2 text-yellow-700">
              <WarningIcon />
              <div>
                <p className="font-medium">Hay estudiantes con notas sin completar</p>
                <p className="text-sm mt-1">Las notas vacías serán calculadas como 0 al guardar</p>
              </div>
            </div>
          </CardBody>
        </Card>
          ...
          <CardBody className="py-4">
            <div className="flex items-start gap-3 text-orange-700">
              <div className="flex-1">
                <p className="font-medium mb-2">⚠️ Los porcentajes no están correctamente distribuidos</p>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mb-3">
                  <div className="bg-white p-2 rounded border">
                    <span className="text-xs text-gray-600">Actividades regulares</span>
                    <p className="font-semibold">{porcentajes.totalRegulares.toFixed(1)}% / 70%</p>
                  </div>
                  <div className="bg-white p-2 rounded border">
                    <span className="text-xs text-gray-600">Evaluación final</span>
                    <p className="font-semibold">{porcentajes.totalFinal.toFixed(1)}% / 30%</p>
                  </div>
                  <div className="bg-white p-2 rounded border">
                    <span className="text-xs text-gray-600">Total</span>
                    <p className="font-semibold">{porcentajes.total.toFixed(1)}% / 100%</p>
                  </div>
                </div>
                <Button
                  color="warning"
      ...
      <Card className="shadow-sm">
        <CardHeader className="pb-3">
          <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-3 w-full">
            <div>
              <h2 className="text-xl font-bold text-gray-800">Actividades de Evaluación</h2>
        ...
        <CardBody className="pt-4">
            ...
            <div className="mb-6">
              <Card className="bg-gray-50 border">
                <CardBody className="p-4">
                  <h3 className="text-lg font-semibold mb-4 text-gray-800">Actividades Configuradas</h3>
                  <div className="overflow-x-auto mb-4">
                    ...
                    >
                      ...
                      <TableBody>
                        {actividades.map((actividad) => (
                          <TableRow key={actividad.id} className={actividad.isFinal ? "bg-blue-50" : "bg-white"}>
                            ...
                            <TableCell className="hidden sm:table-cell">
                              <Chip
                                size="sm"
                                color={actividad.isFinal ? "primary" : "default"}
                                variant="dot"
                              >
                  ...
                  <div>
                    <h4 className="text-md font-semibold mb-3 text-gray-700">Añadir Nueva Actividad</h4>
          ...
          <div>
            <h3 className="text-md font-semibold mb-3 text-gray-700">Distribución Actual de Porcentajes</h3>
            ...
            <div className="bg-gray-100 p-3 rounded-lg">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold">{porcentajes.totalRegulares.toFixed(1)}%</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Evaluación final:</span>
                  <span className="font-semibold">{porcentajes.totalFinal.toFixed(1)}%</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Total:</span>
                  <span className={`font-semibold ${porcentajes.esValido ? 'text-green-600' : 'text-red-600'}`}>
      ...
      <Card className="shadow-sm">
        <CardHeader className="pb-3">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 w-full">
            <div>
              <h2 className="text-xl font-bold text-gray-800">Registro de Calificaciones</h2>
              <p className="text-sm text-gray-600 mt-1">
                Ingresa las calificaciones de cada estudiante para todas las actividades
              </p>
            </div>
              ...
              >
                ...
                <TableBody>
                  {estudiantes.map(estudiante => {
                    const allCells = [
                      <TableCell key="estudiante" className="font-medium sticky left-0 bg-white min-w-[150px] border-r">
                        <div className="flex items-center gap-2">
                      ...
                      <TableCell key="nota-final" className="text-center font-bold">
                        <Chip
                          size="sm"
                          color={
                            parseFloat(calcularNotaFinal(estudiante.id)) >= 3.5 ? "success" :
                            parseFloat(calcularNotaFinal(estudiante.id)) >= 3.0 ? "warning" : "danger"
                          }
                          variant="flat"
                        >
                      ...
                      <TableCell key="reporte" className="text-center">
                        <Tooltip content={`Generar reporte de ${estudiante.nombre_completo}`}>
                          <Button 
                            onPress={() => handleGenerateAreaPDF(estudiante.id, area, calificacionesContext, estudiantes)} 
                            color="warning" 
                            size="sm" 
                            isIconOnly
                            variant="flat"
        ...
        <CardBody className="py-4">
          <div className="flex flex-col sm:flex-row sm:justify-end gap-3">
            ...
            <Button
              ...
              startContent={!loading && !guardandoCalificaciones ? <CheckCircle className="w-4 h-4" /> : null}
            >
              {loading || guardandoCalificaciones ? "Guardando..." : "Guardar Calificaciones"}
            </Button>
          </div>
        </CardBody>
      </Card>
