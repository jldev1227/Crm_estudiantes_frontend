...
export default function ActualizarActividadPage() {
    ...
    const toggleReemplazarFotos = useCallback(() => {
        const confirmar = window.confirm("¿Estás seguro de que deseas eliminar todas las fotos existentes?");
        if (confirmar) {
    ...
    }, [formData, reemplazarFotos, actividad_id, grado_id, area_id, actualizarActividad]);

    // Estados derivados
    const maxRemainingImages = useMemo(() => 10 - uploadedUrls.length, [uploadedUrls.length]);
    const isMaxImagesReached = useMemo(() => uploadedUrls.length >= 10, [uploadedUrls.length]);

    // Condicionales de renderizado
    if (loadingActividad) {
        return <div className="flex justify-center items-center h-64">
            <div className="text-lg font-medium text-gray-700">Cargando información de la actividad...</div>
        </div>;
    }
    ...
    return (
        <div className="space-y-5">
            <div className="flex flex-col gap-1">
                <h1 className="text-3xl text-primary font-bold">Actualizar actividad</h1>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative">
                ...
                <div className="md:sticky md:top-0 md:self-start space-y-4">
                    ...
                    <div>
                        ...
                        {isMaxImagesReached ? (
                            <p className="text-sm text-amber-600 mb-2">Has alcanzado el límite máximo de 10 imágenes</p>
                        ) : (
                            <DropzoneActividad
