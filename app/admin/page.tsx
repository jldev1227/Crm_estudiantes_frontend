...
import { GraduationCap, Users, ChevronRight, PersonStanding, BookOpen, BarChart3, Calendar, Bell, Activity, Award, TrendingUp, Clock } from "lucide-react";
    ...
    <div className="space-y-8">
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-lg text-gray-600 max-w-2xl mx-auto">
          Administra cursos, maestros y calificaciones desde un solo lugar con herramientas intuitivas y potentes.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 2xl:grid-cols-4 gap-6">
        {cards.map((card, index) => {
          ...
          return (
            ...
            >
              <div className={`relative overflow-hidden bg-gradient-to-br ${isHovered ? card.hoverColor : card.color} text-white rounded-3xl p-8 shadow-xl transition-all duration-500 group-hover:shadow-2xl border border-white/20`}>
                {/* Background pattern */}
                <div className="absolute inset-0 opacity-10">
                  <div className={`w-full h-full ${card.bgPattern}`}></div>
                </div>

                {/* Floating elements */}
                <div className="absolute -top-6 -right-6 w-24 h-24 bg-white/10 rounded-full blur-xl group-hover:scale-150 transition-transform duration-700"></div>
      ...
      <div className="bg-white/60 backdrop-blur-sm border border-white/40 rounded-3xl p-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {additionalMetrics.map((metric, index) => (
            <div key={index} className="bg-white/70 border border-gray-200 rounded-xl p-4 hover:bg-white/90 transition-all duration-300 hover:scale-105">
