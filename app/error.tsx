    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      <div className="bg-white p-8 rounded-lg shadow-lg max-w-md w-full">
        <div className="flex items-center justify-center mb-6">
          <div className={`${isPensionError ? "bg-amber-100" : "bg-red-100"} p-3 rounded-full`}>
            {isPensionError ? (
              <svg 
        ...
        <h2 className={`text-2xl font-bold text-center ${isPensionError ? "text-amber-800" : "text-gray-800"} mb-4`}>
          ...
          {isPensionError ? (
            <>
              ...
              <span className="block mt-2">
                Tu pensión escolar no se encuentra activa. Por favor, contacta con el departamento administrativo para regularizar tu situación.
              </span>
            </>
          ) : (
            <>
        ...
        {process.env.NODE_ENV === 'development' && (
          <div className="mt-6 pt-4 border-t border-gray-200">
            <p className="text-xs text-gray-500 mb-1">Información de desarrollo:</p>
            <div className="bg-gray-100 rounded p-2 text-xs overflow-x-auto">
              <p><strong>Mensaje:</strong> {error.message}</p>
              {error.stack && (
                <p className="mt-1"><strong>Origen:</strong> {error.stack.split('\n')[1]?.trim()}</p>
              )}
            </div>
          </div>
        )}
