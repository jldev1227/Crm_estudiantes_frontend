  ...
  return (
    <div className='space-y-6'>
      <div className='space-y-4'>
        <h1 className='text-2xl uppercase font-bold text-blue-600'>Materias asignadas</h1>
        <p>Gestiona tus materias asignadas y realiza el consulta las actividades realizadas para tu curso</p>
      </div>
                ...
                <TableRow key={index}>
                  ...
                  <TableCell>
                    <Button
                      color='primary'
                      isIconOnly
                      onPress={() => route.push(`/estudiante/actividades?area=${area.id}`)}
                    >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {areas?.map((area, index) => (
            ...
            >
              ...
              <div className="p-4">
                <h2 className="text-xl font-semibold text-gray-800">{area.nombre}</h2>
                <p className="text-gray-600 mt-2">Profesor: {area.nombre}</p>
                <div className="mt-4 flex justify-end">
                  <Button onPress={()=>route.push(`/estudiante/actividades?area=${area.id}`)} color='primary' className="flex items-center gap-2">
