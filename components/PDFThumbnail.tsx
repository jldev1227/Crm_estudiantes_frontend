...
const PDFThumbnail: React.FC<PDFThumbnailProps> = ({ url, index }) => {
  ...
  const extractName = (url: string): string => {
    try {
      ...
      return (
        fileName.replace(/-/g, " ").substring(0, 15) +
        (fileName.length > 15 ? "..." : "")
      );
    } catch (e) {
      console.error(e);
  ...
  return (
    <a
      className="h-full w-full border rounded bg-white shadow-sm hover:shadow-md transition-shadow flex flex-col"
      href={`${url}?${process.env.NEXT_PUBLIC_AZURE_KEY}`}
      rel="noreferrer"
      target="_blank"
    >
